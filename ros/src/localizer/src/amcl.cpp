		...
		for (int i = 0; i < particle_num; i++)
		{
			...
			for (int j = 0; j < (int)scan.ranges.size(); j += scan_step)
			{
				if (is_valid_scan_points[j])
				{
					...
					if (r <= range)
					{
						p2 = z_short * (1.0 / (1.0 - exp(-lambda_short * range))) * lambda_short * exp(-lambda_short * r);
						if (p2 > 1.0)
							p2 = 1.0;
					}
					x = r * cos(yaw) + xo;
...
double AMCL::compute_weight_with_dspd(pose_t pose, sensor_msgs::LaserScan scan, int p_index)
{
	double w = 0.0;
	double c = cos(pose.yaw);
	double s = sin(pose.yaw);
...
void AMCL::evaluate_particles_with_dspd(sensor_msgs::LaserScan scan)
{
	...
	for (int i = 0; i < particle_num; i++)
	{
		double weight = compute_weight_with_dspd(particles[i].pose, scan, i);
		particles[i].w *= weight;
		if (i == 0)
		{
			max_particle_likelihood_num = i;
