...
sensor_msgs::PointCloud NDT::get_mean_points_of_ndt_map(std::string frame_id, float dist, float xo, float yo)
{
    ...
    for (int i = 0; i < ndt_map_grid_num; i++)
    {
//        printf("point_num = %d, occ_rate = %lf\n", ndt_map[i].points_num, ndt_map[i].occupancy_rate);
        if (ndt_map[i].points_num >= ndt_map_min_points_num && ndt_map[i].occupancy_rate >= occupancy_rate_threshold)
        {
            if (dist < 0.0)
            {
...
//                printf("x = %f, y = %f, dl = %f\n", ndt_map[i].mean_x, ndt_map[i].mean_y, dl);
                if (dl <= dist)
                {
                    geometry_msgs::Point32 p;
                    p.x = ndt_map[i].mean_x;
...
double NDT::compute_probability2(double dx, double dy, int node)
{
    ...
    if (fabs(det) < 0.00000000000000001) // when computation was to be unstable because of low variances
        return 1.0;
    double d1 = var_yy * dx - var_xy * dy;
    double d2 = -var_xy * dx + var_xx * dy;
    double d = (d1 * dx + d2 * dy) / det;
    return 1.0 / (pow(2.0 * M_PI, 2.0) * sqrt(det)) * exp(-d);
}
...
double NDT::compute_probability(float x, float y)
{
    ...
    {
        return compute_probability2(dx1, dy1, node1);
    }
    else if (n1 < ndt_map_min_points_num && n2 >= ndt_map_min_points_num)
    {
        return compute_probability2(dx2, dy2, node2);
    }
    else
    {
